...
test_1.test.describe("API Integration", () => {
  ...
  ;(0, test_1.test)(
    "should load page content from Strapi API",
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(body).not.toContain("Loading...")
      ;(0, test_1.expect)(body.length).toBeGreaterThan(100) // Should have substantial content
    }
  )
  ;(0, test_1.test)(
    "should have clean console with no API errors",
    async ({ page }) => {
      const consoleErrors = []
      const networkErrors = []
      ...
      ;(0, test_1.expect)(criticalNetworkErrors.length).toBe(0)
    }
  )
  ;(0, test_1.test)(
    "should successfully load data from Strapi via SSR",
    async ({ page }) => {
      ...
      await (0, test_1.expect)(newsletterSection.first()).toBeVisible({
        timeout: 5000,
      })
      ...
      ;(0, test_1.expect)(hasStrapiContent).toBe(true)
    }
  )
  ;(0, test_1.test)(
    "should render Strapi content structure correctly",
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(hasSections || hasComponents).toBe(true)
    }
  )
  ;(0, test_1.test)(
    "should handle Strapi API down gracefully",
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(hasGracefulFallback).toBe(true)
    }
  )
  ;(0, test_1.test)("should retry failed API requests", async ({ page }) => {
    let requestCount = 0
    ...
    ;(0, test_1.expect)(hasContent.length).toBeGreaterThan(100)
  })
  ;(0, test_1.test)(
    "should populate newsletter section from API data",
    async ({ page }) => {
      // Newsletter section should have content from Strapi
      const newsletterHeading = page
        .locator("text=/stay updated|newsletter/i")
        .first()
      await (0, test_1.expect)(newsletterHeading).toBeVisible()
  ...
  ;(0, test_1.test)(
    "should populate contact form from API data",
    async ({ page }) => {
      ...
      const contactHeading = page
        ...
        .first()
      await (0, test_1.expect)(contactHeading).toBeVisible()
      ...
      await (0, test_1.expect)(emailInput).toBeVisible()
      await (0, test_1.expect)(messageInput).toBeVisible()
    }
  )
  ;(0, test_1.test)(
    "should populate FAQ section from Strapi data",
    async ({ page }) => {
      // FAQ section should have content from Strapi
      const faqHeading = page
        .locator("text=/frequently asked|faq|questions/i")
        .first()
      await (0, test_1.expect)(faqHeading).toBeVisible({ timeout: 10000 })
      ...
      ;(0, test_1.expect)(faqButtonCount).toBeGreaterThan(0)
  ...
  ;(0, test_1.test)(
    "should respect locale from Strapi data",
    async ({ page }) => {
      // Content should be in English locale (en)
      const htmlLang = await page.getAttribute("html", "lang")
      ;(0, test_1.expect)(htmlLang).toContain("en")
      ...
      ;(0, test_1.expect)(url).toContain("e2e-test-page")
    }
  )
  ;(0, test_1.test)(
    ...
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(bodyContent).toBeTruthy()
    }
  )
  ;(0, test_1.test)(
    "should load images from Strapi media library",
    async ({ page }) => {
      ...
      if (imageCount > 0) {
        ...
        ;(0, test_1.expect)(src).toBeTruthy()
        // Verify image loads successfully
        const naturalWidth = await firstImage.evaluate(
          (img) => img.naturalWidth
        )
        ;(0, test_1.expect)(naturalWidth).toBeGreaterThan(0)
      }
    }
  )
  ;(0, test_1.test)(
