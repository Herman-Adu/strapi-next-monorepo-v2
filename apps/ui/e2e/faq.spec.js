...
test_1.test.describe("FAQ Accordion", () => {
  ...
  ;(0, test_1.test)("should display FAQ section", async ({ page }) => {
    ...
    ;(0, test_1.expect)(count).toBeGreaterThanOrEqual(5)
  })
  ;(0, test_1.test)(
    "should have all FAQ questions visible",
    async ({ page }) => {
      ...
      const question3 = page
        .getByText(/do you provide ongoing support/i)
        .first()
      await (0, test_1.expect)(question1).toBeVisible()
      await (0, test_1.expect)(question2).toBeVisible()
      await (0, test_1.expect)(question3).toBeVisible()
    }
  )
  ;(0, test_1.test)(
    "should start with accordions collapsed",
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(closedCount).toBeGreaterThanOrEqual(5)
      // Verify no open accordion content initially
      const openContent = page.locator('div.overflow-hidden[data-state="open"]')
      await (0, test_1.expect)(openContent).toHaveCount(0)
    }
  )
  ;(0, test_1.test)("should expand accordion on click", async ({ page }) => {
    ...
    const answerText = page
      ...
      .first()
    await (0, test_1.expect)(answerText).toBeVisible({ timeout: 10000 })
    ...
    await (0, test_1.expect)(openItem).toBeVisible()
  })
  ;(0, test_1.test)(
    "should collapse accordion on second click",
    async ({ page }) => {
      const firstQuestion = page
        .getByRole("button", { name: /what technologies do you use/i })
        .first()
      ...
      await (0, test_1.expect)(answerText).toBeHidden()
      // Should be no open items
      const openItems = page.locator('div.border-b[data-state="open"]')
      await (0, test_1.expect)(openItems).toHaveCount(0)
    }
  )
  ;(0, test_1.test)(
    "should allow multiple accordions open simultaneously",
    async ({ page }) => {
      ...
      const openItems = page.locator('div.border-b[data-state="open"]')
      await (0, test_1.expect)(openItems).toHaveCount(2)
      ...
      const answer1 = page
        ...
        .first()
      await (0, test_1.expect)(answer1).toBeVisible()
    }
  )
  ...
  ;(0, test_1.test)(
    "should handle Space key to toggle accordion",
    async ({ page }) => {
      const firstQuestion = page
  ...
  ;(0, test_1.test)(
    ...
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(
        ["button", "summary"].includes(tagName) || role === "button"
      ).toBe(true)
  ...
  ;(0, test_1.test)(
    "should display all 5 FAQ items from test data",
    async ({ page }) => {
  ...
  ;(0, test_1.test)(
    "should expand accordion with smooth animation",
    async ({ page }) => {
      const firstQuestion = page
        .getByRole("button", { name: /what technologies do you use/i })
        .first()
      ...
      const openContent = openItem
        ...
        .first()
      const finalHeight = await openContent.evaluate((el) => el.offsetHeight)
      ;(0, test_1.expect)(finalHeight).toBeGreaterThan(0)
    }
  )
  ;(0, test_1.test)("should be responsive on mobile", async ({ page }) => {
    ...
    if (answerBox) {
      ;(0, test_1.expect)(answerBox.width).toBeLessThanOrEqual(375)
    }
  })
  ;(0, test_1.test)(
    "should maintain state when scrolling",
    async ({ page }) => {
      const firstQuestion = page
        .getByRole("button", { name: /what technologies do you use/i })
        .first()
      ...
      await (0, test_1.expect)(answerText).toBeVisible()
    }
  )
  ;(0, test_1.test)(
    ...
    async ({ page }) => {
      ...
      ;(0, test_1.expect)(finalState).not.toBe(isVisible)
    }
  )
})
